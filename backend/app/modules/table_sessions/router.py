...
@router.get("/my", response_model=ApiResponse[TableSessionStartResponse])
def get_my_session(
    ...
    return success_response(data=TableSessionStartResponse(
...
@router.post("/start", response_model=ApiResponse[TableSessionStartResponse])
def start_session(
    ...
    """
    return success_response(data=service.start_table_session(db, payload), message="Session started")
...
def request_bill(
    ...
    return success_response(data={"message": "Bill request sent to staff."}, message="Bill request sent")


@router.get("/bill-requests", response_model=ApiResponse[BillRequestListResponse])
def list_bill_requests(
    ...
    return success_response(data={"requests": sessions}, message="Bill requests retrieved")
...
def request_service(
    ...
    return success_response(data={"message": f"Request for {payload.service_type} sent to staff."}, message="Service requested")


@router.get("/service-requests", response_model=ApiResponse[ServiceRequestListResponse])
def list_service_requests(
    ...
    return success_response(data={"requests": requests}, message="Service requests retrieved")
...
def acknowledge_service_request(
    ...
    if not success:
        return success_response(data={"error": "Request not found"}, message="Not found") # Can handle properly via error_response but keeping simple
    return success_response(data={"message": "Service request acknowledged."}, message="Acknowledged")
...
def acknowledge_bill(
    ...
    if not success:
        return success_response(data={"error": "Bill request not found or already acknowledged"}, message="Not found")
    return success_response(data={"message": "Bill request acknowledged."}, message="Acknowledged")
...
def present_bill(
    ...
    if not success:
        return success_response(data={"error": "Session not found"}, message="Not found")
    return success_response(data={"message": "Bill presented to customer."}, message="Presented")
...
def confirm_bill(
    ...
    return success_response(data={"message": "Bill confirmed."}, message="Confirmed")
...
def resolve_service_request(
    ...
    if not success:
        return success_response(data={"error": "Request not found"}, message="Not found")
    return success_response(data={"message": "Request marked as resolved."}, message="Resolved")
...
def change_table(
    ...
    if not success:
        return success_response(data={"error": "Session not found"}, message="Not found")
    return success_response(data={"message": f"Session moved to table {payload.new_table_number}"}, message="Table changed")
