...
def _key(session_id: str, restaurant_id: int) -> str:
    """Build the canonical Redis key for a cart."""
    return f"cart:{session_id}:{restaurant_id}"


def get_cart_raw(
    r: redis_lib.Redis, session_id: str, restaurant_id: int
) -> dict[str, str]:
    """Return the raw hash contents of the cart (item_id -> quantity strings)."""
    return r.hgetall(_key(session_id, restaurant_id))  # type: ignore[return-value]
...
def set_cart_item(
    ...
    key = _key(session_id, restaurant_id)
...
def clear_cart(
    ...
    r.delete(_key(session_id, restaurant_id))
...
def refresh_cart_ttl(
    ...
    r.expire(_key(session_id, restaurant_id), settings.cart_ttl_seconds)
