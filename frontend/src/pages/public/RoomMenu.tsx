...
export default function RoomMenu() {
  ...
  const handleCategorySelect = (categoryId: number | null) => {
    setIsMenuSelectionActive(false); 
  ...
  }, [guestNameInput, restaurantContextId, roomNumber]);
  ...
  const handleRequestService = useCallback(async (serviceType: string, message?: string) => {
    ...
    const endpoint = isBill ? "/room-sessions/my/request-bill" : "/room-sessions/my/request-service";
  ...
  }, [restaurantId, roomNumber, quickServices]);
  ...
  const displayRoomNumber =
    roomNumber && /^\d+$/.test(roomNumber) ? roomNumber.padStart(2, "0") : roomNumber;
        ...
        <div className="mx-auto flex h-16 w-full max-w-[min(72rem,100%)] items-center justify-between gap-3 px-4 sm:px-6">
          <div className="flex min-w-0 items-center gap-3">
            ...
            <div className="min-w-0">
              ...
              <p className="text-[10px] font-bold text-slate-500">{t("menu:room")} {displayRoomNumber}</p>
            </div>
          </div>
          ...
          <div className="absolute bottom-0 right-0 top-0 box-border flex w-full max-w-[min(24rem,100%)] flex-col bg-white shadow-2xl transition-all duration-300 animate-in slide-in-from-right-40">
            ...
            <div className="flex-1 overflow-y-auto">
              ...
              <div className="border-b border-slate-200 px-5 py-5 sm:px-6 sm:py-6">
                ...
                {roomNumber && restaurantId && (
                  <div className="mt-4 space-y-2 rounded-2xl bg-slate-50 p-4 text-sm">
                    <p className="text-slate-600">
                      <span className="font-semibold text-slate-900">{t("menu:room")}:</span> {roomNumber}
                    </p>
              ...
              <div className="py-3">
                {restaurantId && roomNumber && (
                  <Link
                    to={
                      qrAccessKey
                        ? `/menu//room//orders?k=${encodeURIComponent(qrAccessKey)}`
                        : `/menu//room//orders`
                    }
                    onClick={() => setProfileDrawerOpen(false)}
            ...
            <div className="border-t border-slate-200 px-6 py-4 pb-[max(1rem,env(safe-area-inset-bottom))]">
                  <button
                    ...
                    className="w-full rounded-2xl bg-red-50 p-4 text-center font-bold text-red-600 transition hover:bg-red-100"
              >
