...
from sqlalchemy import Boolean, Column, Integer, String, DateTime, Text, Enum, ForeignKey, UniqueConstraint, Index
...
class SuperAdmin(Base):
    """Super Admin user (platform level, no restaurant restriction)"""
...
class Restaurant(Base):
    ...
    password = Column(String(255), nullable=False)
    opening_time = Column(String(8))  # HH:MM:SS
    ...
    __table_args__ = (
        Index("idx_status_active", "subscription_status", "is_active"),
    )
...
class Admin(Base):
    """Admin/Staff users (belongs to restaurants)"""

    __tablename__ = "admin_tbl"

    admin_id = Column(Integer, primary_key=True)
    restaurant_id = Column(Integer, ForeignKey("restaurant_tbl.restaurant_id", ondelete="CASCADE"), nullable=False, index=True)
...
class FieldDefinition(Base):
    """Field definitions for field-level access control"""
...
class RoleFieldPermission(Base):
    """Role-field permissions mapping"""
...
class AuditLog(Base):
    """Audit log for tracking all changes (changed values only)"""

    __tablename__ = "audit_log"

    audit_id = Column(Integer, primary_key=True)
    restaurant_id = Column(Integer, ForeignKey("restaurant_tbl.restaurant_id", ondelete="SET NULL"), index=True)

    # Actor info
    actor_id = Column(Integer, index=True)  # Can be admin_id or super_admin_id
